...
def main(config):
    # 从 YAML 文件中加载所有配置
    all_config = get_all_config(config, 'train')
        ...
        if len(free_gpus) > 0:
            if len(target_gpus) > 0:
                ...
                if len(not_free_ids) != 0:
                    print(f'Target GPU {target_gpus[not_free_ids]} is not free ({gpu_utilizations[not_free_ids]}), waiting for {waiting_time} seconds...')
            ...
            print(f'No free GPU, waiting for {waiting_time} seconds...')
            time.sleep(waiting_time)
...
if __name__ == '__main__':
    # 从 YAML 文件加载配置
    yaml_path = "path/to/your/config.yaml"  # 替换为你的 YAML 文件路径
