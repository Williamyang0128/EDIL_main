...
class DETRVAE(nn.Module):
    ...
    def encode(self, qpos, actions=None, is_pad=None, vq_sample=None):
        ...
        if self.encoder is None:
            ...
            if is_training:
                # print(actions.shape)
                print(torch.tile(actions.unsqueeze(2), (1, 1, 14)).shape)
                # project action sequence to embedding dim, and concat with a CLS token
                action_embed = self.encoder_action_proj(
                    actions
                )  # (bs, seq, hidden_dim)
                # action_embed = self.encoder_action_proj(
                #     torch.tile(actions.unsqueeze(2), (1, 1, 14)).float()
                # )
                # action_embed = self.encoder_action_proj(
                #     F.interpolate(
                #     actions.float().unsqueeze(1), 
                #     size=14, mode='linear', 
                #     align_corners=False
                #     ).squeeze(1)
                # )  # (bs, seq, hidden_dim)
                ...
                print("action_embed shape:", action_embed.unsqueeze(1).shape)
    ...
    def forward(
        self, qpos, image, lang_prompt, env_state, actions=None, is_pad=None, vq_sample=None
    ):
        """
        qpos: batch, qpos_dim
